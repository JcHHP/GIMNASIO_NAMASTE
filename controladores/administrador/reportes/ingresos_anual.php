<?php
    ...
    if(isset($_GET['fecha1']) && isset($_GET['nombre1']) && isset($_GET['monto1']) && isset($_GET['tamaño'])){
        ...
        $reporte->Cell(190,15,'REPORTE DE INGRESOS',0,1,'C');
        
        $reporte->SetFont('Arial','',12);
        $reporte->Ln(4);
        $reporte->MultiCell(190,8,utf8_decode('A continuación se muestran los detalles de los ingresos que se tuvieron en la empresa en el transcurso del año '.$año.', todos los ingresos mostrados en la parte inferior se obtuvieron de los detalles de las matrículas (montos pagados) que tuvieron los clientes durante el transcurso del año mencionado.'),0,'L');
        ...
        for($i=1;$i<=$_GET['tamaño'];$i++){
            ...
            $reporte->Cell(130,8,utf8_decode('     Nueva matrícula de '). utf8_decode($_GET['nombre'.$i]),1,0,'L');
        ...
        $reporte->Cell(160,8,utf8_decode("INGRESO TOTAL DEL AÑO"),1,0,'C');
        ...
        $reporte->OutPut('D',utf8_decode('Ingresos - año '). $año.'.pdf');
